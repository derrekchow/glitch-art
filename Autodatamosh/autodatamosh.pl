...
system("ffmpeg -i $filename.mp4 $filename.avi");
...
if (length $filename) {
	open($infile,'<',$filename.".avi") or die("Could not open '".$filename."': $!");
	open($outfile,'>',$filename."_moshed.avi") or die("Could not open '".$filename."': $!");
}
